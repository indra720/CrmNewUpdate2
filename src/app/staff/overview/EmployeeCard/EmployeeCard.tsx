...
export function EmployeeCard({
  ...
  useEffect(() => {
    try {
      ...
      if (savedCheckInData) {
        const { time, location } = JSON.parse(savedCheckInData);
      ...
      // Reset state if localStorage data is invalid or missing
      setIsCheckedIn(false);
      setCheckInTime(undefined);
      setCurrentCheckInLocation('');
      localStorage.removeItem('checkInData'); // Clear bad data
    }
  }, []);
  ...
  const handleCheckoutSubmit = async () => {
    ...
    // Frontend validation
    if (!checkoutFormData.projectName.trim()) {
      toast({
        title: "Project Name Required",
        description: "Please enter a project name before checking out.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: errorData.error || errorData.detail || "Failed to log your work.",
          variant: "destructive",
        });
      }
    } catch (error) {
